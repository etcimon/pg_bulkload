...
static void _bt_build_callback(Relation index, ItemPointerData *data, Datum *values,
				   ...
				   void *state)
{
	...
	if (tupleIsAlive || buildstate->spool2 == NULL)
		_bt_spool(buildstate->spool, data, values, isnull);
	else
	{
		/* dead tuples are put into spool2 */
		buildstate->havedead = true;
		_bt_spool(buildstate->spool2, data, values, isnull);
	}
...
_bt_buildadd(BTWriteState *wstate, BTPageState *state, IndexTuple itup)
{
		...
		(pgspc < state->btps_full && last_off > P_FIRSTKEY))
	{
		...
		*hii = *ii;
		...
		Assert(BTreeTupleGetNAtts(state->btps_minkey, wstate->index) == 0 ||
			   !P_LEFTMOST((BTPageOpaque) PageGetSpecialPointer(opage)));
		BTreeTupleSetDownLink(state->btps_minkey, oblkno);
	...
	if (last_off == P_HIKEY)
	{
		...
		BTreeTupleSetNAtts(state->btps_minkey, 0, false);
	}
...
_bt_uppershutdown(BTWriteState *wstate, BTPageState *state)
{
	...
	for (s = state; s != NULL; s = s->btps_next)
	{
		...
		else
		{
			...
			Assert(BTreeTupleGetNAtts(s->btps_minkey, wstate->index) == 0 ||
				   !P_LEFTMOST(opaque));
			BTreeTupleSetDownLink(s->btps_minkey, blkno);
	...
	_bt_initmetapage(metapage, rootblkno, rootlevel, false);
